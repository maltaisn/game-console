...
class SoundObject(DataObject):
    ...
    def pack(self) -> PackResult:
        tempo_us = round(bpm_to_beat_us(self.tempo_bpm))
        config = Config(self.file, "", tempo_us, self.octave_adjust,
                        self.merge_midi_tracks, self.time_range, self.channels, False)
        try:
            sound_data = create_sound_data(config)
